...
const VerifyToken = async (req, res, next) => {
  try {
    ...
    if (!authHeader) throw new CustomError(MSG[req.lang].REQUEST_NOT_ALLOW, 400)

    /** check req header */
    const headers = authHeader?.split(' ')
    const token = headers.length > 1 ? headers[1] : null

    if (!token) throw new CustomError(MSG[req.lang].REQUEST_NOT_ALLOW, 400)
    ...
    if (err.message.includes('expired')) return next(new CustomError(MSG[req.lang].TOKEN_EXPIRED, 401))
